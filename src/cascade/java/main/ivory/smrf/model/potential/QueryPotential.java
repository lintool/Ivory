...
//import ivory.data.ApproxProximityPostingsReader;
...
public class QueryPotential extends PotentialFunction {
	...
	public final List<TermNode> termNodes = new ArrayList<TermNode>();
	public final GlobalTermEvidence termEvidence = new GlobalTermEvidence();
	public final Posting curPosting = new Posting();

	public RetrievalEnvironment env = null;
	public ScoringFunction scoringFunction = null;
	public ExpressionGenerator expressionGenerator = null;
	public DocumentNode docNode = null;
	public PostingsReader postingsReader = null;

	public boolean endOfList = true;  	// Whether or not we're at the end of the postings list.
	public int lastScoredDocno = 0;    // Whether or not we're at the end of the postings list.
	...
	public void initialize(List<GraphNode> nodes, GlobalEvidence globalEvidence) throws ConfigurationException {
		...
		} else if (postingsReader instanceof ProximityPostingsReader/* || postingsReader instanceof ApproxProximityPostingsReader*/) {
			termEvidence.set(env.getDefaultDf(), env.getDefaultCf());
		} else {
			termEvidence.set(postingsReader.getPostingsList().getDf(), postingsReader.getPostingsList().getCf());
		}
	...
	public int getNextCandidate() {
		//System.out.println("QueryPotential::getNextCandidate");
