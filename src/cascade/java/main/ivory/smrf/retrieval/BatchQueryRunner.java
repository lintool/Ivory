...
public class BatchQueryRunner {
	public static final Logger LOGGER = Logger.getLogger(BatchQueryRunner.class);

	public DocnoMapping mDocnoMapping = null;
	public FileSystem mFs = null;
	public String mIndexPath = null;
	public RetrievalEnvironment mEnv = null;

	public Map<String, String> mQueries = new LinkedHashMap<String, String>();
	private Map<String, Node> mImportanceModels = new LinkedHashMap<String, Node>();
	public Map<String, Node> mModels = new LinkedHashMap<String, Node>();
	private Map<String, Node> mDocscores = new LinkedHashMap<String, Node>();
	public Map<String, Node> mExpanders = new HashMap<String, Node>();
	public Set<String> mStopwords = new HashSet<String>();
	private Map<String, Map<String, Double>> mJudgments = new HashMap<String, Map<String, Double>>();

	public Map<String, QueryRunner> mQueryRunners = new LinkedHashMap<String, QueryRunner>();
	...
	public BatchQueryRunner(String[] args, FileSystem fs) throws ConfigurationException {
		...
		boolean fixedNumOfBins= true; boolean arrayOfBits = true; int approxParam = 0;
		try {
			fixedNumOfBins = Boolean.parseBoolean(args[0]);
			arrayOfBits = Boolean.parseBoolean(args[1]);
			approxParam = Integer.parseInt(args[2]);
			String[] xmlArgs = new String[args.length-3];
			for(int i = 3; i<args.length; i++) xmlArgs[i-3] = args[i];
			args=xmlArgs;
			approx = true;
			//System.out.println("Approx configuration");
		} catch (Exception e) {
			//System.out.println("Exact configuration");
		}
		...
		try {
			...
			//if(approx)
			//	mEnv.initializeApprox(true, fixedNumOfBins, arrayOfBits, approxParam);
