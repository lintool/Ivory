...
public class UnorderedWindowExpressionGenerator extends ExpressionGenerator {
	 ...
	 * default constructor 
	 */
	public UnorderedWindowExpressionGenerator() {
		mWidth = 4;
	}
	 ...
	 * @param width
	 */
	public UnorderedWindowExpressionGenerator( int width ) {
		mWidth = width;
	}
	...
	public String getExpression(String terms) {
		String [] tokens = TextTools.getTokens( terms );
		int numTerms = tokens.length;
		if( mWidth == 0 ) {
			return "#uw( " + terms + " )";
		}
		return "#uw" + ( numTerms * mWidth ) + "( " + terms + " )";
	}
