...
public class BuildTermDocVectors extends PowerTool {
	...
	private static class MyMapper extends MapReduceBase implements
		...
		public void close() throws IOException {
			...
			if (mDocLengths.size() == 0) {
				throw new RuntimeException("Doc Length table Empty !!!!");
			}
			long bytesCnt = 0;
	...
	public int runTool() throws Exception {
		...
		String tokenizerModel = conf.get("Ivory.TokenizerModel");
		...
		// TODO: This seems like a special purpose hack?
		if (tokenizerModel != null) {
			DistributedCache.addCacheFile(new URI(tokenizerModel), conf);
			if (conf.get("Ivory.Lang").equals("english")) {
				DistributedCache.addCacheFile(new URI(conf.get("Ivory.EngVocab")), conf);
			} else {
				DistributedCache.addCacheFile(new URI(conf.get("Ivory.GerVocab")), conf);
			}
		}
		...
		String dlFile = env.getDoclengthsData();
		if (fs.exists(new Path(dlFile))) {
	...
	private void writeDoclengthsData(int collectionDocCount) throws IOException {
		...
		String dlFile = env.getDoclengthsData();
		String inputPath = env.getDoclengthsDirectory();
