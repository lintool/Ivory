...
public class EnAr_TREC02 {
  ...
  private static Map<Integer,float[]> expectedMAPs = new HashMap<Integer,float[]>();{ 
    expectedMAPs.put(0, new float[]{0.3020f, 0.2497f, 0.2475f});  // "one2none" -> phrase,1best,10best
    expectedMAPs.put(1, new float[]{0.2824f, 0.2489f, 0.2488f});  // "one2one" -> phrase,1best,10best
    expectedMAPs.put(2, new float[]{0.2931f, 0.2422f, 0.2551f});  // "one2many" -> phrase,1best,10best
  };
  ...
  private static Map<Integer, String[]> Gridbest_AP = phrase_AP;
  ...
  public static void verifyAllResults(Set<String> models,
    ...
    for (int heuristic=0; heuristic <= 2; heuristic++) {
      g.put("en-ar.phrase_10-0-0-100_" + heuristic, new GroundTruth(Metric.AP, numTopics, phrase_AP.get(heuristic), expectedMAPs.get(heuristic)[0]));
