...
public class RunQueryEngine {
  ...
  static Configuration parseArgs(String[] args) {
    ...
    try {
      ...
      e.printStackTrace();
      System.out
          .println("usage: ivory.sqe.retrieval.RunQueryEngine bow [index-path] [queries-file]  [qrels] [src-lang] [tokenizer-model-file]");
      System.out
          .println("usage: ivory.sqe.retrieval.RunQueryEngine clir [index-path] [queries-file]  [qrels] [src-lang] [tokenizer-model-file] [vocab-f-file] [vocab-e-file] [ttable-f2e-file] (lex-prob-threshold) (cum-prob-threshold)");
      System.out
          .println("usage: ivory.sqe.retrieval.RunQueryEngine phrase [index-path] [queries-file] [qrels] [src-lang] [tokenizer-model-file] [grammar-file] (H1=yes|no) (H2=yes|no) (H3=sum|max|avg)");
      System.out
          .println("usage: ivory.sqe.retrieval.RunQueryEngine phrase2 [index-path] [queries-file] [qrels] [src-lang] [tokenizer-model-file] [vocab-f-file] [vocab-e-file] [ttable-f2e-file] [grammar-file] (H3=sum|max|avg) (lex-prob-threshold) (cum-prob-threshold)");
  ...
  static void eval(QueryEngine qe, Configuration conf) {
    ...
    for (String qid : results.keySet()) {
      ...
      float p10 = (float) RankedListEvaluator.computePN(10, results.get(qid), mapping, qrels
          .getReldocsForQid(qid));
      LOG.info(qid + ":" + results.get(qid).length + "," + qrels.getReldocsForQid(qid).size()
          + " => " + ap + "," + p10);
      apSum += ap;
      p10Sum += p10;
    }
