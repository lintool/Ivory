...
public class CLWordQueryGenerator implements QueryGenerator {
  ...
  public void init(FileSystem fs, Configuration conf) throws IOException {
    if (conf.get(Constants.fVocabPath) == null || conf.get(Constants.eVocabPath) == null
        || conf.get(Constants.Language) == null || conf.get(Constants.TokenizerData) == null) {
      throw new RuntimeException("Missing configuration parameters: "
          + conf.get(Constants.fVocabPath) + "," + conf.get(Constants.eVocabPath) + ","
          + conf.get(Constants.Language) + "," + conf.get(Constants.TokenizerData));
    }
    fVocab_f2e = (VocabularyWritable) HadoopAlign.loadVocab(
        new Path(conf.get(Constants.fVocabPath)), fs);
    eVocab_f2e = (VocabularyWritable) HadoopAlign.loadVocab(
        new Path(conf.get(Constants.eVocabPath)), fs);
    ...
    tokenizer = TokenizerFactory.createTokenizer(fs, conf.get(Constants.Language), conf
        .get(Constants.TokenizerData), fVocab_f2e);
