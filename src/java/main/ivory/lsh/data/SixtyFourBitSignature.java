 ...
 * 
 */
public class SixtyFourBitSignature extends Signature {// implements
                                                      // WritableComparable<BitsSignature64> {
  private byte[] bits;
  ...
  public void set(int i, boolean sign) {
    if (!sign)
      getBits()[i / 8] &= ~(1 << (7 - i % 8)); // clear
    else
      getBits()[i / 8] |= 1 << (7 - i % 8); // set
  }
