...
public class BitsSignatureTable implements Writable {
  ...
  public void set(Signature[] signs, int[] nos, int nElements) {
    if (signs.length == 0)
      throw new RuntimeException("invalid size of signatures: " + signs.length);
    if (nos.length == 0)
      throw new RuntimeException("invalid size of docnos: " + nos.length);
    if (signs.length != nos.length)
      throw new RuntimeException("size mismatch: " + signs.length + "\t" + nos.length);
    if (nElements <= 0 || nElements > signs.length)
      throw new RuntimeException("invalid size: " + nElements);
   ...
   * public void readFields(DataInput in) throws IOException {
   * 
   * nSignatures = in.readInt(); if(signaturesArray== null || signaturesArray.length < nSignatures)
   * signaturesArray = new SixtyFourBitSignature[nSignatures]; for(int i = 0; i < nSignatures; i++){
   * signaturesArray[i] = new SixtyFourBitSignature(); signaturesArray[i].readFields(in); }
   * if(docNosArray== null || docNosArray.length < nSignatures) docNosArray = new int[nSignatures];
   * for(int i = 0; i < nSignatures; i++){ docNosArray[i] = in.readInt(); } }
   */
  ...
  public void write(DataOutput out) throws IOException {
    ...
    for (int i = 0; i < nSignatures; i++)
      signaturesArray[i].write(out);
    for (int i = 0; i < nSignatures; i++)
      out.writeInt(docNosArray[i]);
  }
