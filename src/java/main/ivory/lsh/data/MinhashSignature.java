...
/**
 * 
 * A version of BitsSignature specifically tuned for 64 bits. Uses an array of 8 bytes instead of
 * the Bits object.
 * 
...
public class MinhashSignature extends Signature {// implements WritableComparable<BitsSignature64> {
  ...
  /**
   * @param start first index to be included in sub-signature
   * @param end last index to be included in sub-signature
   * @return return a new BitsSignature object, containing the bits of this object from
  ...
  /*
   * public int compareTo(Object obj) { BitsSignature64 other = (BitsSignature64) obj;
   * if(size()!=other.size()){ throw new
   * RuntimeException("Cannot compare different sized signatures!"); } for(int i=0;i<size();i++){
   * if(!get(i) && other.get(i)){ return -1; }else if(get(i) && !other.get(i)){ return 1; } } return
   * 0; }
   */
  ...
  public int compareTo(MinhashSignature other) {
    ...
    /*
     * if(size()!=other.size()){ throw new
     * RuntimeException("Cannot compare different sized signatures!"); }
     */
    int myBit, otherBit;
    for (int i = 0; i < size(); i++) {
      myBit = get(i);
   ...
   * @return return a new ArrayListOfIntsWritable object, containing the ints of this object from
   *         <code>start</code> to <code>end</code>
   */
