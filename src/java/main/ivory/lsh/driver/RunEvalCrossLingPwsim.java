...
public class RunEvalCrossLingPwsim extends PwsimEnvironment implements Tool {
  ...
  private static int printUsage() {
    System.out
        .println("usage: [targetlang-dir] [srclang-dir] [num-bits] [type-of-signature] [num-perms] [overlap-size] [window-size] [max-dist] [sample-size] [mode]\nIf you want to run full pwsim on all document pairs, mode=all, otherwise mode=sample\n");
    return -1;
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    config.set("Ivory.CollectionName", targetEnv.readCollectionName() + "_"
        + srcEnv.readCollectionName());
    ...
    if (chunkOverlapSize < slidingWindowSize) {
      System.out
          .println("Error: [overlap-size] cannot be less than [window-size] for algorithm's correctness!");
      return -1;
    }
    ...
    // sample from non-English weighted int doc vectors: these are needed by BruteForcePwsim, which
    // finds the ground truth pairs for the sample using dot product of doc vectors
    if (!hdfs.exists(new Path(sampleWtdIntDocVectorsPath))) {
      int frequency = (srcCollSize / sampleSize);
      String[] sampleArgs = { wtdIntDocVectorsPath, sampleWtdIntDocVectorsPath, "100",
          frequency + "" };
    ...
    if (!hdfs.exists(sampleDocnosPath)) {
      ...
      try {
        docno2DocVectors = SequenceFileUtils.readFileIntoMap(new Path(sampleWtdIntDocVectorsPath
            + "/part-00000"));
    ...
    if (mode.equals("all")) {
      String[] clSlidingArgs = { getFileNameWithPars(targetLangDir, "Tables"),
          getFileNameWithPars(targetLangDir, "PWSimCollection"), slidingWindowSize + "",
          maxHammingDistance + "" };
      CLSlidingWindowPwsim.main(clSlidingArgs);
    } else { // pwsim on sample documents only
      String[] clSlidingArgs = { getFileNameWithPars(targetLangDir, "Tables"),
          getFileNameWithPars(targetLangDir, "PWSimCollectionFiltered"), slidingWindowSize + "",
          maxHammingDistance + "", sampleDocnosPath.toString() };
      CLSlidingWindowPwsim.main(clSlidingArgs);
    }
    ...
    if (mode.equals("all")) {
      ...
      String[] filterArgs = { getFileNameWithPars(targetLangDir, "PWSimCollection"),
          filterPWSimResultsPath, sampleDocnosPath.toString(), maxHammingDistance + "",
          numResults + "" };
    ...
    String groundTruthOutput = (numResults > 0) ? (targetLangDir + "/groundtruth_T=" + T + "_topN=" + numResults)
        : (targetLangDir + "/groundtruth_T=" + T + "_topN=all");
    if (!hdfs.exists(new Path(groundTruthOutput))) {
      String[] evalArgs = { "docvectors", getFileNameWithPars(targetLangDir, "IntDocs"),
          groundTruthOutput, sampleWtdIntDocVectorsPath + "/part-00000", T + "", numResults + "" };
    ...
    // String[] evalArgs = {"signatures", getFileNameWithPars(targetLangDir,"SignaturesRandom"),
    // signatureOutput, srcLangDir+"/sample-signatures-random_D=1000/part-00000", "400", "-1"};
    // BruteForcePwsim.main(evalArgs);
    //
    //		
    // dist = 436;
    // signatureOutput = targetLangDir+"/pwsim_dist="+dist;
    // String[] evalArgs2 = {"signatures", getFileNameWithPars(targetLangDir,"SignaturesRandom"),
    // signatureOutput, srcLangDir+"/sample-signatures-random_D=1000/part-00000", "436", "-1"};
