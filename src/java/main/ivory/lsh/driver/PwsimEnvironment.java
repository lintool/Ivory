...
public abstract class PwsimEnvironment extends Configured {
  ...
  // Signatures
  public static String permutationType; // how to permute signatures: by bits ("bit") or by blocks
                                        // ("block")
  public static String signatureType; // the type of signature: "simhash", "minhash", or "random"
  public static int numOfPermutations; // number of permutation tables (i.e., Q : number of
                                       // permutations to use in randomized pwsim algorithm)
  ...
  // Sliding Window
  public static int slidingWindowSize; // window size of similarity comparisons (i.e., B: beam size
                                       // parameter for randomized pwsim algorithm)
  ...
  // Batch
  public static int batchIndexKeyLength; // length of indexing key in bits for batch and hybrid
                                         // algorithms
  public static int numBatchFiles; // number of batch files (i.e., MapReduce jobs) for batch
                                   // algorithm
  ...
  public static String getFileNameWithPars(String dir, String fileName) throws Exception {
    ...
    if (fileName.equals("TermDocs")) {
      ...
      if (numBatchFiles > 0) {
        return dir + "/map-signatures-" + signatureType + "_L=" + batchIndexKeyLength + "_Q="
            + numOfPermutations + "_D=" + numOfBits + "_batch=" + numBatchFiles;
      } else {
        return dir + "/map-signatures-" + signatureType + "_L=" + batchIndexKeyLength + "_Q="
            + numOfPermutations + "_D=" + numOfBits;
      }
      /*
       * }else if(fileName.equals("P-Bnd-BatchSignaturesMap")){ return dir +
       * "/p-bnd-map-signatures-"
       * +signatureType+"_L="+batchIndexKeyLength+"_Q="+numOfPermutations+"_D="+numOfBits;
       */
    } else if (fileName.equals("PWSim")) {
      return dir + "/similardocs_D=" + numOfBits + "_Q=" + numOfPermutations + "_B="
          + slidingWindowSize; // for backward compatibility. may remove later.

    } else if (fileName.equals("PWSimCollection")) {
      if (batchIndexKeyLength > 0)
        return dir + "/similardocs_hybrid_" + signatureType + "_maxdst=" + maxHammingDistance
            + "_D=" + numOfBits + "_Q=" + numOfPermutations + "_B=" + slidingWindowSize + "_L="
            + batchIndexKeyLength + (pairwiseWithinChunk ? "_pw" : "");
      else
        return dir + "/similardocs_coll_" + signatureType + "_maxdst=" + maxHammingDistance + "_D="
            + numOfBits + "_Q=" + numOfPermutations + "_B=" + slidingWindowSize;

    } else if (fileName.equals("PWSimCollectionFiltered")) {
      if (batchIndexKeyLength > 0)
        return dir + "/similardocs_hybrid_" + signatureType + "_maxdst=" + maxHammingDistance
            + "_D=" + numOfBits + "_Q=" + numOfPermutations + "_B=" + slidingWindowSize + "_L="
            + batchIndexKeyLength + (pairwiseWithinChunk ? "_pw" : "") + "-filtered_top"
            + numResults;
      else
        return dir + "/similardocs_coll_" + signatureType + "_maxdst=" + maxHammingDistance + "_D="
            + numOfBits + "_Q=" + numOfPermutations + "_B=" + slidingWindowSize + "-filtered_top"
            + numResults;

    } else if (fileName.equals("P-PWSimCollection")) {
      if (batchIndexKeyLength > 0)
        return dir + (withBoundaries ? "/p-b" : "/p") + "-similardocs_hybrid_" + signatureType
            + "_maxdst=" + maxHammingDistance + "_D=" + numOfBits + "_Q=" + numOfPermutations
            + "_B=" + slidingWindowSize + "_L=" + batchIndexKeyLength
            + (pairwiseWithinChunk ? "_pw" : "");
      else
        return dir + (withBoundaries ? "/p-b" : "/p") + "-similardocs_coll_" + signatureType
            + "_maxdst=" + maxHammingDistance + "_D=" + numOfBits + "_Q=" + numOfPermutations
            + "_B=" + slidingWindowSize;
      ...
      if (chunkOverlapSize > 0)
        return dir
            ...
            + "_Q="
            + numOfPermutations
            + ((batchIndexKeyLength > 0) ? ("_L=" + batchIndexKeyLength)
                : ("_B=" + slidingWindowSize));
      else
        return dir
            + "/eval_similardocs_"
            + ((numBatchFiles > 0) ? "batch" : "hybrid")
            + "_"
            + signatureType
            + "_maxdst="
            + maxHammingDistance
            ...
            + "_Q="
            + numOfPermutations
            + ((batchIndexKeyLength > 0) ? ("_B=" + slidingWindowSize + "_L=" + batchIndexKeyLength)
                : "") + ((numBatchFiles == 0 && pairwiseWithinChunk) ? "_pw" : "");
      ...
      return dir + "/similardocs_batch_" + signatureType + "_maxdst=" + maxHammingDistance + "_D="
          + numOfBits + "_Q=" + numOfPermutations + "_L=" + batchIndexKeyLength;

    } else if (fileName.equals("P-Bnd-PWSimBatch")) {
      return dir + "/p-bnd-similardocs_batch_" + signatureType + "_maxdst=" + maxHammingDistance
          + "_D=" + numOfBits + "_Q=" + numOfPermutations + "_L=" + batchIndexKeyLength;

    } else if (fileName.equals("Tables")) {
      if (batchIndexKeyLength > 0) {
        return dir + "/tablesIndexed_" + signatureType + "_D=" + numOfBits + "_keylen="
            + batchIndexKeyLength + "_Q=" + numOfPermutations;
      } else {
        return dir + "/tables_" + signatureType + "_D=" + numOfBits + "_V=" + chunkOverlapSize
            + "_Q=" + numOfPermutations;
      }

    } else if (fileName.equals("P-Tables")) {
      if (batchIndexKeyLength > 0) {
        return dir + (withBoundaries ? "/p-b" : "/p") + "-tablesIndexed_" + signatureType + "_D="
            + numOfBits + "_keylen=" + batchIndexKeyLength + "_Q=" + numOfPermutations;
      } else {
        return dir + (withBoundaries ? "/p-b" : "/p") + "-tables_" + signatureType + "_D="
            + numOfBits + "_V=" + chunkOverlapSize + "_Q=" + numOfPermutations;
      }
      ...
      return dir + "/eval-seealso_coll_perm=bit_maxdst=" + maxHammingDistance + "_D=" + numOfBits
          + "_Q=" + numOfPermutations + "_B=" + slidingWindowSize;

    } else if (fileName.equals("DuplicateSets")) {
      return dir + "/sets_similardocs_coll_perm=bit_maxdst=" + maxHammingDistance + "_D="
          + numOfBits + "_Q=" + numOfPermutations + "_B=" + slidingWindowSize;
    }
