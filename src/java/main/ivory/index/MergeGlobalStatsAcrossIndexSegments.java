...
public class MergeGlobalStatsAcrossIndexSegments extends PowerTool {
	...
	private static final Logger sLogger = Logger
			.getLogger(MergeGlobalStatsAcrossIndexSegments.class);

	private static class MyMapper extends MapReduceBase implements
			Mapper<LongWritable, Text, Text, PairOfIntLong> {

		private static final PairOfIntLong sDfCf = new PairOfIntLong();
	...
	public int runTool() throws Exception {
		...
		sLogger.info("Job: ComputeNumberOfTermsAcrossIndexSegments");
		...
		sLogger.info("Job: MergeGlobalStatsAcrossIndexSegments");
	...
	public static void main(String[] args) throws Exception {
		...
		sLogger.info("Merging global statistics across index segments...");
		sLogger.info(" CollectionName: " + collectionName);
		sLogger.info(" OutputPath: " + outputPath);
		sLogger.info(" DfThreshold: " + dfThreshold);

		sLogger.info(" IndexPaths: ");
		StringBuffer sb = new StringBuffer();
		for (int i = 3; i < args.length; i++) {
			sLogger.info("    Adding" + args[i]);
			sb.append(args[i]);
			if (i != args.length - 1)
				sb.append(",");
		}
