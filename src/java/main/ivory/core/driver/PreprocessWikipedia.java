...
public class PreprocessWikipedia extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    String collectionLang = null, tokenizerModel = null, collectionVocab = null, fVocab_f2e = null, eVocab_f2e = null, fVocab_e2f, eVocab_e2f = null, ttable_f2e = null, ttable_e2f = null;
    ...
    // normalize (optional) and convert weighted term doc vectors into int doc vectors for
    // efficiency
    startTime = System.currentTimeMillis();
    ...
    if (mode == MONO_LINGUAL) {
      ...
      LOG.info("Job BuildTargetLangWeightedIntDocVectors finished in "
          + (System.currentTimeMillis() - startTime) / 1000.0 + " seconds");
      if (finalNumDocs > 0) {
        LOG.info("Changed doc count from " + env.readCollectionDocumentCount() + " to = "
            + finalNumDocs);
        env.writeCollectionDocumentCount(finalNumDocs);
      }
      // set Property.CollectionTermCount to the size of the target vocab. since all docs are
      // translated into that vocab. This property is read by WriteRandomVectors via
      // RunComputeSignatures.
      ...
      LOG.info("Changed term count to : " + env.readCollectionTermCount() + " = "
          + engVocabH.size());
      env.writeCollectionTermCount(engVocabH.size());
    }

    LOG.info("Preprocessing job finished in " + (System.currentTimeMillis() - preprocessStartTime)
        / 1000.0 + " seconds");
