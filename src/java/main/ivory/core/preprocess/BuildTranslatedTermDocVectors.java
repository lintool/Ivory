...
/**
 * Translates term doc vectors in foreign language (e.g. German) into target language (e.g. English)
 * using the CLIR technique discussed in Combining Bidirectional Translation and Synonymy for
 * Cross-Language Information Retrieval, SIGIR'06, Jianqiang Wang and Douglas W. Oard.
 * 
 * @author ferhanture
 * 
 */
...
public class BuildTranslatedTermDocVectors extends PowerTool {
  ...
  private static class MyMapperTrans extends MapReduceBase implements
    ...
    int MIN_SIZE = 0; // minimum document size, to avoid noise in Wikipedia due to stubs/very short
                      // articles etc. this is set via Conf object
    ...
    public void map(IntWritable docno, TermDocVector doc,
      ...
      if (!language.equals("english") && !language.equals("en")) {
        docno.set(docno.get() + 1000000000); // to distinguish between the two collections in the
                                             // PWSim sliding window algorithm
      }
  ...
  private static class DataWriterMapper extends NullMapper {
    ...
    public void run(JobConf conf, Reporter reporter) throws IOException {
      ...
      if (!fs2.exists(new Path(fFile)) || !fs2.exists(new Path(eFile))
          || !fs2.exists(new Path(e2fttableFile)) || !fs2.exists(new Path(termsFile))
          || !fs2.exists(new Path(dfByIntFile))) {
      ...
      DefaultFrequencySortedDictionary dict = new DefaultFrequencySortedDictionary(new Path(env
          .getIndexTermsData()), new Path(env.getIndexTermIdsData()), new Path(env
          .getIndexTermIdMappingData()), fs2);
      DfTableArray dfTable = new DfTableArray(new Path(dfByIntFile), fs2);

      HMapIFW transDfTable = CLIRUtils.translateDFTable(eVocab_e2f, fVocab_e2f, en2DeProbs, dict,
          dfTable);

      SequenceFile.Writer writer = SequenceFile.createWriter(fs2, conf, new Path(transDfFile),
