...
public class OpenNLPTokenizer extends ivory.core.tokenize.Tokenizer {
  ...
  public void configure(Configuration mJobConf) {
    ...
    VocabularyWritable vocab;
    try {
      vocab = (VocabularyWritable) HadoopAlign.loadVocab(new Path(mJobConf
          .get("Ivory.CollectionVocab")), fs);
      setVocab(vocab);
    } catch (Exception e) {
  ...
  public void setLanguageAndStemmer(String l) {
    ...
    Class stemClass;
    try {
      stemClass = Class.forName("org.tartarus.snowball.ext." + languages[lang] + "Stemmer");
  ...
  public boolean isDiscard(String token) {
    return ((lang == ENGLISH && isStopwordRemoval && stopwords.contains(token))
        || delims.contains(token) || token.length() < MIN_LENGTH || token.length() > MAX_LENGTH);
    // return (delims.contains(token) || token.length() < MIN_LENGTH || token.length() >
    // MAX_LENGTH);
  }
  ...
  public static void main(String[] args) throws IOException, ClassNotFoundException,
    ...
    // eTokenizer.setTokenizer(FileSystem.get(new Configuration()), new
    // Path("/Users/ferhanture/edu/research/programs/opennlp-tools-1.4.3/models/EnglishTok.bin"));
    ...
    // VocabularyWritable engVocabH = (VocabularyWritable) HadoopAlign.loadVocab(new
    // Path(srcVocabFile), localFs);
    //
    //
    ...
    OpenNLPTokenizer eTokenizer = new OpenNLPTokenizer();
    eTokenizer.setTokenizer(FileSystem.get(new Configuration()), new Path(
        "/Users/ferhanture/edu/research/programs/opennlp-tools-1.4.3/models/GermanTok.bin"));
    ...
    VocabularyWritable engVocabH = (VocabularyWritable) HadoopAlign.loadVocab(
        new Path(srcVocabFile), localFs);
