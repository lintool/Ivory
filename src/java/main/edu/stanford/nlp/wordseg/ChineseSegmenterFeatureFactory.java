...
public class ChineseSegmenterFeatureFactory<IN extends CoreLabel> extends FeatureFactory<IN> implements Serializable {
	 ...
	 */
	public Collection getCliqueFeatures(PaddedList<IN> cInfo, int loc, Clique clique) {
	...
	protected Collection featuresC(PaddedList<IN> cInfo, int loc) {
...
class TagAffixDetector {
	...
	public TagAffixDetector(SeqClassifierFlags flags) {
		...
		String ccPath;
		String adPath;
		FSDataInputStream ccStream = null; 
		FSDataInputStream adStream = null; 
		
		System.err.println("INFO: Flags.Conf "+flags.conf);
			...
			if (flags.useASBCChar2 || flags.useHKChar2 || flags.useMSRChar2) {
				...
				try{
					if(flags.conf!=null){
						...
						ccStream = fs.open(new Path(ccPath));
						adStream = fs.open(new Path(adPath));
					}
				} catch (IOException e) {
					System.err.println("Cannot read from HDFS--ccPath, adPath");
					e.printStackTrace();
				} 
			} else {
		...
		if(flags.conf!=null){
			cc = new CorpusChar(ccStream);
			aD = new affDict(adStream);			
		}else{
			System.err.println("INFO: TagAffixDetector: useChPos=" + flags.useChPos +
					" | useCTBChar2=" + flags.useCTBChar2 + " | usePKChar2=" + flags.usePKChar2+"CONF="+flags.conf);
