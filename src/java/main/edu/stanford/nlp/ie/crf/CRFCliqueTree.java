...
//      marginal[label] = factorTables[position].logProbEnd(label);     // remove: for debugging only
  ...
  /**
   * Returns the probability for the given labels (indexed using classIndex), where the
   * last label corresponds to the label at the specified position.  For instance if you called
  ...
  /**
   * Gives the probability of a tag at a single position conditioned on
   * a sequence of previous labels.
   *
