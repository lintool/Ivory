...
public abstract class GrammaticalStructure extends TreeGraph {
  ...
  private List<TypedDependency> getDeps(boolean getExtra, Filter<TypedDependency> f) {
    ...
    assert(roots.size() == 1);
    TreeGraphNode root = new TreeGraphNode(new Word("ROOT"));
    root.setIndex(0);
    Iterator<TypedDependency> iterator = roots.iterator();
    if (iterator.hasNext()) {

      TreeGraphNode rootDep = (iterator.hasNext() ?
                               iterator.next().gov() : null);
      basicDep.add(new TypedDependency(ROOT, root, rootDep));
    }
